...
WORD2CHANGE[' as needed ' ] = ['[\s|.|,|;]+p[.]?r[.]?n[\s|.|,|;|:]+', ' s needed ']
...
def _DIRECTION_QUERY(TIME_INTERVAL):
    DIRECTION_QUERY = """WITH batch_date AS ( -- Updated 2021-03-12, get estimated ship date for prioritization
                      SELECT batch_id,  
                      MIN(date) batch_date
                      FROM  source_pillpack_core.batch_ship_dates
                      WHERE date BETWEEN CURRENT_TIMESTAMP() - INTERVAL '6 day' AND CURRENT_TIMESTAMP + interval '7 day'
                      GROUP BY batch_id)
                      ...
                      batch_date batch_date -- Updated 2021-03-12, get estimated ship date for prioritization
                      ...
                      LEFT JOIN source_pillpack_core.users urs ON docs.user_id = urs.id  -- Updated 2021-03-12
                      LEFT JOIN batch_date bd ON bd.batch_id = urs.batch_id  -- Updated 2021-03-12
                      ...
                      AND docs.queue = 'Archive' -- Updated 2021-03-12 for multi-batch""".format(TIME_INTERVAL)
...
def main(TIME_INTERVAL):
    ...
    data.to_csv(PATH+'/MB_Results/snapshots_'+TIME+'.csv', index=False)
